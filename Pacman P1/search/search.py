...
#rom searchAgents import manhattanHeuristic
...
def depthFirstSearch(problem):
    ...
    while not S.isEmpty():
        ...
        d = dep[len(dep)-1]
        d = d[0]
        if(problem.isGoalState(d)):
            return [array[1] for array in dep][1:]
        if d not in visited:
            visited.append(d)
            for nextnode in problem.getSuccessors(d):
...
def breadthFirstSearch(problem):
    ...
    while not Q.isEmpty():
        ...
        b = brd[len(brd)-1]
        b = b[0]
        if problem.isGoalState(b):
            return [array[1] for array in brd][1:]
        if b not in visited:
           visited.append(b)
           for nextnode in problem.getSuccessors(b):
               if nextnode[0] not in visited:
                  nextnodepath = brd[:]
                  nextnodepath.append(nextnode)
                  Q.push(nextnodepath) 
...
def uniformCostSearch(problem):
    ...
    while not PQ.isEmpty():
        ...
        for nextState, action, cost in problem.getSuccessors(state):
            totalCost = cost1 + cost
            PQ.push((nextState, actions + [action], totalCost), totalCost)
