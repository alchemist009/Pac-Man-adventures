...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            if noisyDistance == None:
                possiblePos[self.getJailPosition()] = 1 
            if emissionModel[trueDistance] > 0:
                possiblePos[p] = emissionModel[trueDistance] * self.beliefs[p]

        "*** END YOUR CODE HERE ***"

        possiblePos.normalize()
        self.beliefs = possiblePos
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        updBeliefs = util.Counter()
        for oldPos in self.legalPositions:    
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                updBeliefs[newPos] += self.beliefs[oldPos] * prob
        
        updBeliefs.normalize()
        self.beliefs = updBeliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        #print 'particlecount', num
        while particleCount > 0:
            for l in self.legalPositions:
                self.particleList.append(l)
                particleCount -= 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            for p in self.particleList:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                updBeliefs[p] += emissionModel[trueDistance]
                #if emissionModel[trueDistance] > 0:
                 #   updBeliefs[p] = emissionModel[trueDistance] * beliefs[p]
            if updBeliefs.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                self.particleList = []
                for i in range(self.numParticles):
                    self.particleList.append(util.sample(updBeliefs))
        else:
            if noisyDistance == None:
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particleList:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            updList.append(util.sample(newPosDist))
        self.particleList = updList
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        jointPositions = random.sample(positions, len(positions))
        num = 1
        ...
        for particle in self.particleList:
            ...
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] != None:
                    ...
                    weight *= emissionModels[ghost][trueDistance]
                #if emissionModel[trueDistance] > 0:
                 #   updBeliefs[p] = emissionModel[trueDistance] * beliefs[p]
                elif noisyDistances[ghost] == None:
                    particle = self.getParticleWithGhostInJail(particle, ghost)
            updBeliefs[particle] += weight
