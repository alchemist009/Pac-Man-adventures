...
def enhancedFeatureExtractorDigit(datum):
    ...
    #a = datum.getPixels()
    #features = util.Counter()
    #print "Digit height", DIGIT_DATUM_HEIGHT
    #print "Digit width", DIGIT_DATUM_WIDTH
    ...
    for x in range(0,DIGIT_DATUM_HEIGHT/2):
    ...
    for x in range(DIGIT_DATUM_HEIGHT/2,DIGIT_DATUM_HEIGHT):
        for y in range(DIGIT_DATUM_WIDTH/2,DIGIT_DATUM_WIDTH):
            if datum.getPixel(x, y) == 0:
                features[(x,y)] -= 0
            else:
                features[(x,y)] += 1
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if (datum.getPixel(x, y-1) > 0 and datum.getPixel(x-1,y)>0 and datum.getPixel(x-1,y)>0 and datum.getPixel(x,y-1)>0):
                features[(x,y)] -= 0
            else:
                features[(x,y)] += 1
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            while(x < DIGIT_DATUM_WIDTH - 2 and datum.getPixel(x+1,y)>0):
                features[(x,y)] += 1
                x = x+1
    ...
    for y in range(DIGIT_DATUM_HEIGHT/2, DIGIT_DATUM_HEIGHT):
        for x in range(DIGIT_DATUM_WIDTH):
            if datum.getPixel(x,y) > 0:
                botcount += 1
    feat = 0
    if (topcount - botcount) > 20 or (topcount - botcount) < -20:                   #difference in pixel count in upper and lower halves
        feat = 1
        
    pixrat = 0
    if (topcount/botcount) > 2.0:                                                       #ratio of on pixeks in upper and lower half
        pixrat = 1
        
    
    def loop_dfs(x,y):                                                                    #simple dfs to check for continuous sequence of off pixels
        if x in range(0, DIGIT_DATUM_WIDTH - 1):
            if y in range(0, DIGIT_DATUM_HEIGHT - 1):
                if datum.getPixel(x,y) > 0:
                    return True
            else:
                return False
        ...
        mark[(x,y)] = True
        return (loop_dfs(x+1, y) and loop_dfs(x-1,y) and loop_dfs(x,y+1) and loop_dfs(x,y-1))
    
    #features["pixrat"] = pixrat
    ...
    centcount = 0
    notcentcount = 0
    for y in range(DIGIT_DATUM_HEIGHT/4, DIGIT_DATUM_HEIGHT * (3/4)):                               #pixel count in center region
        for x in range(DIGIT_DATUM_WIDTH/4, DIGIT_DATUM_WIDTH * (3/4)):
            if datum.getPixel(x,y) > 0:
                centcount += 1
    ...
    features["pix"] = [pixrat, centcount, feat]
    #features["c"] = centcount - notcentcount
    
    
    pixcount = 0
    for x in range(DIGIT_DATUM_HEIGHT/2,DIGIT_DATUM_HEIGHT):                                #pixel count in top left quadrant
        for y in range(0,DIGIT_DATUM_WIDTH/2):
    ...
    q4 = 0            
    for x in range(DIGIT_DATUM_HEIGHT/2, DIGIT_DATUM_HEIGHT):
        for y in range(DIGIT_DATUM_WIDTH, 0):
            if datum.getPixel(x, y) == 1:
                q4 += 1
    
    qdiff = ((q1 + q3) - (q2 + q4))                                                 #difference in pixel count between alternate quadrants
    qcount = 0            
    if qdiff > 20 or qdiff < -20: 
        qcount += 1
    ...
    for x in range(DIGIT_DATUM_HEIGHT):                                                     #finding contiguous off pixels to check for loops
        for y in range(DIGIT_DATUM_WIDTH):
            mark = {}
            if datum.getPixel(x, y) == 0:
                if loop_dfs(x,y) == True:
                    lcount += 1
                    break
                
                    
    features["loopcheck"] = lcount      
...
def enhancedPacmanFeatures(state, action):
    ...
    newFood = newstate.getFood().asList()
    oldFood = state.getFood().asList()
    #newFood = newFood.asList()                            
    ghostPos = state.getGhostPositions()
    gstate = newstate.getGhostStates()
    scareTime = [g.scaredTimer for g in gstate] 
    foodCount = newstate.getNumFood()
    pacPos = newstate.getPacmanPosition()
    ...
    if foodCount:
        food = lambda x : util.manhattanDistance(x, pacPos)                             #lambda function to calculate all food distances
        for f in newFood:
            fdist.append(food(f))
        if fdist:
            fmin = min(fdist)
    ...
    if fmin < count:                                                                    #distance to closest food
        features["foodheur"] = 1.0/(1.0 + fmin)
    else:
        features["foodheur"] = 5
    ...
    #else:
     #   features["foodheur"] = 666
      #  return features
    gcount = 666
    for g in ghostPos:                                                                  #distance to closest ghost
        gmin = (util.manhattanDistance(pacPos, g))
        gcount = min(gmin,gcount)
        #if g.scareTime > 20:
         #   return float('inf')
    #gmin = min(gdist)

    features["closest ghost"] = gcount
    return features
